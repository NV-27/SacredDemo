...
class CrossValidationSplitter(BaseEstimator, TransformerMixin):
    ...
    cv_seed: int, optional, defailt = None
    ...
    def _get_splitter(self, X: pd.DataFrame, y: pd.Series):
        ...
        if self.cv_strategy == "kfold":
            splitter = KFold(
                n_splits=self.n_folds, shuffle=self.shuffle, random_state=self.cv_seed)
...
if __name__ == "__main__":
    cv = CrossValidationSplitter("kfold", 10, split_column="ZN")

    splitter = cv.transform(X, y)
